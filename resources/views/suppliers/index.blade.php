    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white shadow-sm sm:rounded-lg p-6">
            <div class="flex justify-between items-center mb-4">
                <h2 class="font-semibold text-xl">{{ __('Suppliers') }}</h2>
                <a href="{{ route('suppliers.create') }}" class="btn btn-primary">{{ __('New Supplier') }}</a>
            </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody>
                    @forelse($suppliers as $supplier)
                        <tr class="border-b">
                            <td class="py-2">{{ $supplier->company_name }}</td>
                            <td class="py-2">{{ $supplier->contact_person }}</td>
                            <td class="py-2">{{ $supplier->email }}</td>
                            <td class="py-2">{{ $supplier->phone }}</td>
                            <td class="py-2">
                                <a href="{{ route('suppliers.show', $supplier) }}" class="text-indigo-600">{{ __('View') }}</a>
                                <a href="{{ route('suppliers.edit', $supplier) }}" class="ms-2 text-indigo-600">{{ __('Edit') }}</a>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="5" class="py-4">{{ __('No suppliers yet.') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 border-b border-gray-200">
                    <form action="{{ route('suppliers.index') }}" method="GET" class="flex gap-4">
                        <div class="flex-1">
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search suppliers by company, contact, or email..." 
                            ...
                            @forelse($suppliers as $supplier)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="text-sm text-gray-500">Since {{ $supplier->created_at->format('M Y') }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-900 truncate max-w-xs" title="{{ $supplier->address }}">
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-10 text-center text-gray-500">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-lg font-medium text-gray-900">No suppliers found</p>
                                            <p class="text-sm text-gray-500">Get started by adding a new supplier.</p>
